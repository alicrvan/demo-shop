...
import io.techmo.product_managment.excpetion.ProductAlreadyExistException;
import io.techmo.product_managment.persistence.domain.Product;
import io.techmo.product_managment.persistence.repository.ProductRepository;
...
class ProductServiceTest {
    ...
    void removeProduct() {
        ...
        Mockito.when(productRepository.deleteByProductNumber(productNum)).thenReturn(Optional.of(product));

        String result = productService.removeProduct(productNum);

        assertEquals("Product with " + productNum + " number has been deleted", result);
